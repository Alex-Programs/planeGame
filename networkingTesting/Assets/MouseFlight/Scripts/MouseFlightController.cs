...
namespace MFlight
{
    ...
    public class MouseFlightController : MonoBehaviour
    {
        ...
        //ALEX STUFF
        public bool externallySetHasAuthority = false;
        ...
        private void Update()
        {
            if (externallySetHasAuthority == false)
            {
                Debug.Log("No authority in controller, not running");
                return;
            }
            if (useFixed == false)
                UpdateCameraPos();
        ...
        private void FixedUpdate()
        {
            if (externallySetHasAuthority == false)
            {
                Debug.Log("No authority in controller loop1, not running");
                return;
            }
            if (useFixed == true)
                UpdateCameraPos();
